...
const CATEGORY_LINKS = [
    {
        ...
        labels: { ja: "全身", ko: "전신", "zh-hant": "全身", es: "Cuerpo completo", fr: "Corps entier" },
        descriptions: {
    ...
    },
    {
        ...
        labels: { ja: "胸", ko: "가슴", "zh-hant": "胸部", es: "Pecho", fr: "Pectoraux" },
        descriptions: {
    ...
    },
    {
        ...
        labels: { ja: "背中", ko: "등", "zh-hant": "背部", es: "Espalda", fr: "Dos" },
        descriptions: {
    ...
    },
    {
        ...
        labels: { ja: "肩", ko: "어깨", "zh-hant": "肩部", es: "Hombros", fr: "Épaules" },
        descriptions: {
            ja: "プレス、レイズ、安定性の種目一覧",
    ...
    },
    {
        ...
        labels: { ja: "腕", ko: "팔", "zh-hant": "手臂", es: "Brazos", fr: "Bras" },
        descriptions: {
            ja: "カール、トライセプス、前腕の種目",
            ko: "컬, 삼두, 전완 운동",
    ...
    },
    {
        ...
        labels: { ja: "脚", ko: "하체", "zh-hant": "腿部", es: "Piernas", fr: "Jambes" },
        descriptions: {
    ...
    },
    {
        ...
        labels: { ja: "体幹", ko: "코어", "zh-hant": "核心", es: "Core", fr: "Tronc" },
        descriptions: {
            ja: "腹筋、回旋、体幹安定の種目",
            ko: "복근, 회전, 코어 안정화 운동",
                    ...
                    <div class="dashboard-panel">
                        ...
                        <div class="dashboard-panel-grid">
                            ...
                            <article class="dashboard-mini-card">
                                ...
                                <strong class="metric-value">${escapeHtml(localeText({ ja: "体重 / 年齢", ko: "체중 / 나이", "zh-hant": "體重 / 年齡", es: "Peso / edad", de: "Körpergewicht / Alter" }))}</strong>
                                <strong class="metric-value">${escapeHtml(localeText({ ja: "体重 / 年齢", ko: "체중 / 나이", es: "Peso / edad", fr: "Poids / âge" }))}</strong>
                            ...
                            <article class="dashboard-mini-card">
                                ...
                                <span class="metric-subvalue">${escapeHtml(localeText({ ja: "種目ごとの効き方を把握", ko: "운동별 자극 부위 확인", "zh-hant": "查看每個動作的刺激部位", es: "Estímulo por ejercicio", de: "Zielmuskulatur je Übung prüfen" }))}</span>
                                <span class="metric-subvalue">${escapeHtml(localeText({ ja: "種目ごとの効き方を把握", ko: "운동별 자극 부위 확인", es: "Estímulo por ejercicio", fr: "Travail musculaire par exercice" }))}</span>
                            </article>
                            <article class="dashboard-mini-card">
                                <span class="metric-label">${escapeHtml(t("compareFlow"))}</span>
                                <strong class="metric-value">${escapeHtml(t("relatedExercises"))}</strong>
                                <span class="metric-subvalue">${escapeHtml(localeText({ ja: "同カテゴリを続けて閲覧", ko: "같은 카테고리 이어보기", "zh-hant": "連續查看同分類動作", es: "Seguir en la misma categoría", de: "In derselben Kategorie weitersehen" }))}</span>
                                <span class="metric-subvalue">${escapeHtml(localeText({ ja: "同カテゴリを続けて閲覧", ko: "같은 카테고리 이어보기", es: "Seguir en la misma categoría", fr: "Continuer dans la même catégorie" }))}</span>
                            </article>
                        </div>
                        <div class="dashboard-spotlight-list">
                            <div class="dashboard-spotlight-heading">
                                <span class="metric-label">${escapeHtml(localeText({ ja: "今すぐ見比べる", ko: "바로 비교하기", "zh-hant": "立即比較", es: "Comparar ahora", de: "Jetzt vergleichen" }))}</span>
                                <span class="metric-label">${escapeHtml(localeText({ ja: "今すぐ見比べる", ko: "바로 비교하기", es: "Comparar ahora", fr: "Comparer maintenant" }))}</span>
                            </div>
        ...
        <section class="container section-band">
            ...
            <div class="category-overview-grid">
                ${libraryData.sections.map((section) => {
                    ...
                    const description = categoryMeta?.description || localeText({ ja: "関連種目をまとめて確認", ko: "관련 운동을 한 번에 확인", "zh-hant": "集中查看相關訓練動作", "zh-hans": "集中查看相关训练动作", es: "Revisar ejercicios relacionados", fr: "Voir les exercices liés" });
...
function enhanceExercisePage(main) {
    ...
    decorateDataContainer(
        ...
        measurementKind === "reps"
            ? localeText({ ja: "レベル別の平均レップ数を先に確認できます。", ko: "레벨별 평균 반복 횟수를 먼저 확인할 수 있습니다.", "zh-hant": "可先查看各等級的平均次數。", "zh-hans": "可先查看各等级的平均次数。", es: "Consulta primero las repeticiones medias por nivel.", de: "Prüfe zuerst die durchschnittlichen Wiederholungen je Level." })
            : localeText({ ja: "レベル別の1RM目安を先に確認できます。", ko: "레벨별 1RM 기준을 먼저 확인할 수 있습니다.", "zh-hant": "可先查看各等級的 1RM 參考。", "zh-hans": "可先查看各等级的 1RM 参考。", es: "Consulta primero la referencia de 1RM por nivel.", de: "Prüfe zuerst die 1RM-Referenz je Level." })
            ? localeText({ ja: "レベル別の平均レップ数を先に確認できます。", ko: "레벨별 평균 반복 횟수를 먼저 확인할 수 있습니다.", es: "Consulta primero las repeticiones medias por nivel.", fr: "Consulte d'abord les répétitions moyennes par niveau." })
            : localeText({ ja: "レベル別の1RM目安を先に確認できます。", ko: "레벨별 1RM 기준을 먼저 확인할 수 있습니다.", es: "Consulta primero la referencia de 1RM por nivel.", fr: "Consulte d'abord la référence 1RM par niveau." })
    );
    ...
    const libraryIntro = existingIntro || htmlToElement(`
        <p class="section-intro">
            ${escapeHtml(localeText({
                ...
                ko: "같은 카테고리와 다른 부위의 운동을 비교하면서 다음 페이지로 이동할 수 있습니다.",
            ...
            ${escapeHtml(detectLocale() === "ko"
                ...
                : detectLocale() === "es"
                    ...
                    : detectLocale() === "fr"
                        ? "Compare des exercices de la même catégorie et d'autres zones avant de passer à la page suivante."
...
function enhanceContentPage(main) {
    ...
    libraryContainer.classList.add("section-band", "library-band");
    libraryTitle.classList.add("section-title--database");
    libraryTitle.textContent = libraryTitle.textContent || localeText({ ja: "データベースから続けて探す", ko: "데이터베이스에서 계속 찾기", "zh-hant": "從資料庫繼續探索", es: "Seguir explorando la base de datos", de: "In der Datenbank weiterstöbern" });
    libraryTitle.textContent = libraryTitle.textContent || localeText({ ja: "データベースから続けて探す", ko: "데이터베이스에서 계속 찾기", es: "Seguir explorando la base de datos", fr: "Continuer dans la base de données" });
        ...
        <section class="container section-band performance-snapshot-band">
            ...
            <div class="performance-signal-grid">
                ${renderPerformanceSignalCard(
                    baselineLine?.label || localeText({ ja: "基礎ライン", ko: "입문 기준", "zh-hant": "初學者基準", es: "Referencia principiante", de: "Anfänger-Referenz" }),
                    baselineLine?.label || localeText({ ja: "基礎ライン", ko: "입문 기준", es: "Referencia principiante", fr: "Repère débutant" }),
                    baselineLine?.primary || t("dataAvailable"),
                    baselineLine?.secondary || t("levelAverageAvailable"),
                    true
                )}
                ${renderPerformanceSignalCard(
                    intermediateLine?.label || localeText({ ja: "中級ライン", ko: "중급 기준", "zh-hant": "中級者基準", es: "Referencia intermedia", de: "Fortgeschrittenen-Referenz" }),
                    intermediateLine?.label || localeText({ ja: "中級ライン", ko: "중급 기준", es: "Referencia intermedia", fr: "Repère intermédiaire" }),
                ...
                ${renderPerformanceSignalCard(
                    localeText({ ja: "比較ビュー", ko: "비교 보기", "zh-hant": "比較視圖", "zh-hans": "比较视图", es: "Vista comparativa", de: "Vergleichsansicht" }),
                    localeText({ ja: "比較ビュー", ko: "비교 보기", es: "Vista comparativa", fr: "Vue comparative" }),
...
function getPrimaryMuscles(muscles, main) {
    ...
    const primary = muscles.find((item) => {
        return item.label.includes("主働") || item.label.includes("主動") || item.label.includes("主动") || item.label.includes("主要") || item.label.includes("주동") || item.label.includes("Ziel");
        return item.label.includes("主働") || item.label.includes("主動") || item.label.includes("주동") || item.label.toLowerCase().includes("principal");
    });
...
function formatExerciseCount(count) {
    return localeText({ ja: `${count}種目`, ko: `${count}개 운동`, "zh-hant": `${count} 個動作`, es: `${count} ejercicios`, de: `${count} Übungen` });
}

function formatMuscleCount(count) {
    return localeText({ ja: `${count}筋群`, ko: `${count}개 근육`, "zh-hant": `${count} 個肌群`, es: `${count} músculos`, de: `${count} Muskeln` });
    return localeText({ ja: `${count}種目`, ko: `${count}개 운동`, es: `${count} ejercicios`, fr: `${count} exercices` });
}

function formatMuscleCount(count) {
    return localeText({ ja: `${count}筋群`, ko: `${count}개 근육`, es: `${count} músculos`, fr: `${count} muscles` });
}
