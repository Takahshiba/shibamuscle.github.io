...
${pages.map((page) => {
        return `            <a class="discovery-card" href="${escapeAttribute(page.file)}">
                <span class="discovery-type">${escapeHtml(locale === "de" ? (page.type === "comparison" ? "Vergleich" : "Ziel") : locale === "fr" ? (page.type === "comparison" ? "Comparatif" : "Objectif") : locale === "es" ? (page.type === "comparison" ? "Comparativa" : "Objetivo") : page.type === "comparison" ? "Comparison" : "Intent")}</span>
                <h3>${escapeHtml(page.heading)}</h3>
                <p>${escapeHtml((page.intro || [])[0] || page.description || "")}</p>
                <span class="discovery-link">${locale === "ko" ? "페이지 보기" : locale === "es" ? "Ver página" : locale === "fr" ? "Voir la page" : locale === "de" ? "Seite ansehen" : "ページを見る"}</span>
            </a>`;
    }).join("\n")}
        </div>
    </section>`;
